...
data = pd.read_excel('../Data/pixels_v2.xlsx', sheet_name=0)
...
for s in range(len(scenes)):
    ...
    for c in scenes[s]['Pixel Class'].unique():
        dict[c] = scenes[s][scenes[s]['Pixel Class'] == c][['Y-location', 'X-location']].values
...
#begin processing loop for hmrf 
for s in range(len(scenes)):
    scene_num = s+1
...
#save scores to a csv with column labels image, score
df = pd.DataFrame.from_dict(hmrf_scores, orient='index')
df.columns = ['score']
...
gmm_scores = {}
#begin gmm processing loop
for s in range(len(scenes)):
    scene_num = s+1
    gmm_path = f'../GMM/result/segmentation/scene{scene_num}/'
    gt_pixels = classes[s]
    for image in images:
        path = gmm_path + str(scene_num) + image
        # read in the image
        try:
            img = plt.imread(path)
            # convert image to grayscale
            ...
            score = (2*correct - n)/n
            #add the score to the dictionary
            gmm_scores[str(scene_num) + image] = score

        except:
            pass #if the image doesn't exist, skip it

#save gmm scores to csv
df = pd.DataFrame.from_dict(gmm_scores, orient='index')
df.columns = ['score']
df.to_csv('gmm_scores.csv')